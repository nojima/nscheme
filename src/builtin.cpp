    ...
    auto f = [func, name_symbol](Context* ctx, size_t n_args) {
        if (n_args != 1) {
            throw std::runtime_error(
                name_symbol.toString() + ": Invalid number of arguments.");
        }
...
void registerFunction2(std::unordered_map<Symbol, Value>* variables,
                       Allocator* allocator,
                       SymbolTable* symbol_table,
                       const std::string& name,
...
void registerBuiltinFunctions(std::unordered_map<Symbol, Value>* variables,
    ...
    registerFunction1(variables, allocator, symbol_table, "not",
        [](Context*, Value obj) {
            return Value::fromBoolean(!obj.asBoolean());
        });

    variables->insert(std::make_pair(
        symbol_table->intern("+"),
        Value::fromPointer(allocator->make<CFunctionObject>(sum, "+"))));

    variables->insert(std::make_pair(
        symbol_table->intern("*"),
        Value::fromPointer(allocator->make<CFunctionObject>(prod, "*"))));

    registerFunction2(variables, allocator, symbol_table, "-",
        [](Context*, Value a, Value b) {
    ...
    registerFunction1(variables, allocator, symbol_table, "pair?",
        [](Context*, Value obj) {
            return Value::fromBoolean(isPair(obj));
        });
    ...
    registerFunction1(variables, allocator, symbol_table, "car",
        [](Context*, Value pair) {
            if (!isPair(pair))
    ...
    registerFunction1(variables, allocator, symbol_table, "cdr",
        [](Context*, Value pair) {
            if (!isPair(pair))
